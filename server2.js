...
if(secret=='read'){
  ...
  function readStaff(staff_pk,callback){
    ...
    input.on('data', function(data) {
      ...
      while (index > -1) {
        ...
        setTimeout(callback(staff_pk), 2000);
      }
    });
  ...
  function encryptstaff(plain,callback){
    ...
    var fs = require("fs"),
      lineReader = require('line-reader'),
      filename = "server2_plain.txt",
      encode = "utf8";
    var input = fs.createReadStream('server2_plain.txt');
    var remaining = '';
  ...
  function readPailler_cipher1(paillier_cipher1,callback){
    var fs = require("fs"),
      lineReader = require('line-reader'),
      filename = "paillier_cipher1.txt",
      encode = "utf8";
    var input = fs.createReadStream('paillier_cipher1.txt');
    var remaining = '';
    //read data line-by-line
    input.on('data', function(data) {
      remaining += data;
      ...
      while (index > -1) {
        ...
        //console.log('Line: ' + line);
        //console.log('typeof: ' + typeof(line));
        paillier_cipher1 = line;
        index = remaining.indexOf('\n');
        setTimeout(callback(paillier_cipher1), 2000);
      }
    });
  }
  //main function!
  ...
  readStaff(staff_pk,function(response){
    ...
    encryptstaff(plain,function(response){
      plain = response;
      readPailler_cipher1(paillier_cipher1,function(response){
        paillier_cipher1=response;
        //console.log('staff_pk :'+staff_pk);
        //console.log('plain :'+plain);
        //console.log('paillier_cipher1 :'+paillier_cipher1);
        //console.log('typeof1 :'+typeof paillier_cipher1);
        KeyPairGenerator(1024,staff_pk);
        paillier_cipher2=key['public_key'].raw_encrypt(Number(plain),1080);//about 32bits, input 40000000000
        //console.log('paillier_cipher2 :'+paillier_cipher2);
        //console.log('typeof2 :'+typeof paillier_cipher2.toString());
        //cipher multiplication
        var cipher_mul=key['public_key'].createBNOp(paillier_cipher1,paillier_cipher2.toString());
